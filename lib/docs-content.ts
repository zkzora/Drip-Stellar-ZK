    ...
    sections: [
      ...
      },
      {
        ...
        steps: [
          ...
          {
            title: "Receiver generates a proof locally",
            body: "The stream receiver opens the proof drawer (amount and salt auto-fill from localStorage or an optional link), enters the threshold to prove, and clicks Generate Proof. Noir.js and Barretenberg (bb.js) run in the browser — no data leaves the device. The proof takes ~5–10 seconds and is 14592 bytes. The proof button is receiver-only.",
          },
          {
            title: "Receiver shares a proof code",
            body: "Under Share with a verifier, the receiver copies a compact share code (or downloads a .json) bundling the proof, stream ID, and threshold. The amount and salt are never included, so the code is safe to send to anyone — alongside the /verify link.",
          },
          {
            title: "Anyone verifies on the /verify page",
            body: "A third party — no wallet, no Drip account — opens /verify, pastes the share code, and clicks Verify on Stellar. The page runs a read-only verify_income_proof simulation (funding a throwaway Friendbot account for the source). Stellar's BN254 host functions check the UltraHonk proof on-chain and return Income Verified or Not Verified — without revealing the amount.",
          },
        ],
      },
      {
      ...
      sections: [
        {
          ...
          items: [
            ...
            { question: "Do I need to share my stream amount?",     answer: "No. The share code you send a verifier contains the proof, stream ID, and threshold — never the amount or salt. The verifier only learns whether your stream meets the threshold." },
            { question: "How does someone verify my proof?",        answer: "As the receiver, generate a proof and copy the share code from Share with a verifier, then send it plus the /verify link to whoever needs to check your income. They open /verify, paste the code, and click Verify on Stellar." },
            { question: "Does a verifier need a wallet or account?", answer: "No. The /verify page needs no wallet and no Drip account. It runs a read-only on-chain simulation using a throwaway Friendbot-funded account, so anyone with the share code can confirm the result." },
            { question: "What is the proof link vs the share code?", answer: "The proof link contains the salt and is for the receiver's own convenience (auto-filling the proof form on another device) — never give it to a verifier. The share code contains no salt or amount and is the thing you send to verifiers." },
